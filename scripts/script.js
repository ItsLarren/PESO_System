    ...
    function calculateDemographicStatistics(applicants) {
        ...
        applicants.forEach(applicant => {
            ...
            if (gender === 'male') {
                ...
                stats.unknown++;
                console.log('Unknown gender format:', applicant.SEX); 
            }
...
function initializeProgramFileUpload() {
    ...
    // ========================================================
    // FEIS SYSTEM CREDITS AND PROTECTION
    // Developed by: Engr. Yumul Larren Joy D., Computer Engineer
    // Copyright © 2025 Engr. Yumul Larren Joy D. All Rights Reserved.
    // ========================================================
    (function() {
        'use strict';
        
        const FEIS_SYSTEM_INFO = {
            name: 'Fernandino Employment Information System',
            version: '2.0',
            developer: 'Engr. Yumul Larren Joy D., Computer Engineer',
            copyright: '© 2025 Engr. Yumul Larren Joy D. All Rights Reserved.',
            client: 'City Government of San Fernando, Pampanga',
            department: 'Public Employment Service Office',
            contact: 'Professional License: Computer Engineer',
            license: 'Licensed for use by City Government of San Fernando, Pampanga'
        };
        
        console.log('%c' + '='.repeat(70), 'color: #3498db; font-weight: bold;');
        console.log('%cFERNANDINO EMPLOYMENT INFORMATION SYSTEM (FEIS) v' + FEIS_SYSTEM_INFO.version, 
                    'color: #e74c3c; font-size: 16px; font-weight: bold;');
        console.log('%cDeveloped by: ' + FEIS_SYSTEM_INFO.developer, 
                    'color: #2c3e50; font-weight: bold;');
        console.log('%c' + FEIS_SYSTEM_INFO.copyright, 
                    'color: #c0392b; font-style: italic;');
        console.log('%cLicensed to: ' + FEIS_SYSTEM_INFO.client, 
                    'color: #27ae60;');
        console.log('%c' + '='.repeat(70), 'color: #3498db; font-weight: bold;');
        
        setInterval(() => {
            const creditsElement = document.getElementById('system-credits');
            if (!creditsElement) {
                restoreCreditsFooter();
            }
            
            const styleCheck = document.createElement('div');
            document.body.appendChild(styleCheck);
            const watermarkStyle = window.getComputedStyle(styleCheck, '::after').content;
            document.body.removeChild(styleCheck);
            
            if (watermarkStyle === 'none' || watermarkStyle === 'normal') {
                restoreCSSWatermark();
            }
        }, 3000);
        
        function restoreCreditsFooter() {
            const footer = document.createElement('div');
            footer.id = 'system-credits';
            ...
            `;
            footer.innerHTML = `
                <span>FEIS v${FEIS_SYSTEM_INFO.version} ${FEIS_SYSTEM_INFO.copyright}</span>
                <span>Developer: ${FEIS_SYSTEM_INFO.developer}</span>
                <span>${FEIS_SYSTEM_INFO.license}</span>
            `;
            document.body.appendChild(footer);
        }
        
        function restoreCSSWatermark() {
            const style = document.createElement('style');
            style.id = 'feis-watermark-style';
            style.innerHTML = `
                body::after {
                    content: "FEIS v${FEIS_SYSTEM_INFO.version} - ${FEIS_SYSTEM_INFO.developer}";
                    position: fixed;
                    bottom: 0;
                    right: 0;
                    background: #333;
        ...
        const originalExportFunction = window.exportToExcel || function() {};
        window.exportToExcel = function(...args) {
            const result = originalExportFunction(...args);
            
            if (args[0] && args[0].push) {
                args[0].push({
                    'FIELD': 'SYSTEM INFORMATION',
                    'VALUE': `Generated by ${FEIS_SYSTEM_INFO.name} v${FEIS_SYSTEM_INFO.version}`
                });
                args[0].push({
                    'FIELD': 'DEVELOPER',
                    'VALUE': FEIS_SYSTEM_INFO.developer
                });
                args[0].push({
                    'FIELD': 'COPYRIGHT',
                    'VALUE': FEIS_SYSTEM_INFO.copyright
                });
            }
        ...
        window.__FEIS = Object.freeze(FEIS_SYSTEM_INFO);
        Object.defineProperty(window, '__FEIS', {
            configurable: false,
            writable: false,
            enumerable: false
        });
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const dashboard = document.querySelector('.welcome-card') || 
            ...
            if (dashboard) {
                ...
                badge.innerHTML = `
                    ...
                    <strong>System Developer:</strong> ${FEIS_SYSTEM_INFO.developer}<br>
                    <small>${FEIS_SYSTEM_INFO.name} v${FEIS_SYSTEM_INFO.version}</small>
                `;
                dashboard.appendChild(badge);
            }
            
            document.title = `FEIS v${FEIS_SYSTEM_INFO.version} | ${document.title}`;
    ...
    setInterval(() => {
        ...
        if (heightDiff > threshold && !devToolsOpen) {
            devToolsOpen = true;
            console.log('%c⚠️ DevTools Detected! System credits are protected.', 
